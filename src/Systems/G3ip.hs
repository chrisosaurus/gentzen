...
left_and' [And l r] sequent = do
    ...
    sequent <- return $ add_exps_lhs [l, r] sequent
    return sequent
...
right_and' [And l r] sequent = do
    ...
    sequent_l <- return $ add_exp_rhs l sequent
    sequent_r <- return $ add_exp_rhs r sequent
...
left_or' [Or l r] sequent = do
    ...
    sequent_l <- return $ add_exp_lhs l sequent
    sequent_r <- return $ add_exp_lhs r sequent
    return (sequent_l, sequent_r)
...
right_or1' [Or l r] sequent = do
    ...
    sequent <- return $ add_exp_rhs l sequent
    return sequent
...
right_or2' [Or l r] sequent = do
    sequent <- remove_exp_rhs (Or l r) sequent
    sequent <- return $ add_exp_rhs r sequent
    return sequent
...
left_implies' [Implies l r] sequent@(Sequent lhs rhs) = do
    sequent_l <- return $ Sequent lhs [l]
    sequent_r <- remove_exp_lhs (Implies l r) sequent
    sequent_r <- return $ add_exp_lhs r sequent_r
    return (sequent_l, sequent_r)
...
right_implies' [Implies l r] sequent = do
    ...
    sequent <- return $ add_exp_lhs l sequent
    sequent <- return $ add_exp_rhs r sequent
    return sequent
